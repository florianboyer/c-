...
//on rentre les infos dans la classe personne que l'on a creer au prealable dans un autre fichier
	vectContact.push_back(Contact(nom,prenom,age));
...
void affiche()
{
	for (int nombreDeContact = 0; nombreDeContact < vectContact.size(); ++nombreDeContact)
	{
		cout<<Contact::nom<<" ";
		cout<<Contact::prenom<<" ";
		cout<<Contact::age<<endl;	
	}
...
void recherche()
{
	...
	if (choixRecherche==prenomRechercher)
	{
		...
		while(listeDesContacts<vectContact.size()){
			//si jamais on trouves une correspodance on affiche les infos de la personne
			if(premiereLettre::prenom.vectContact[0] = lettreRechercher){
				cout<<Contact::nom<<" ";
				cout<<Contact::prenom<<" ";
				cout<<Contact::age<<endl;	
			}
			listeDesContacts++ //on rajoute un a personne pour continuer a l'element suivant 
		}
	}
